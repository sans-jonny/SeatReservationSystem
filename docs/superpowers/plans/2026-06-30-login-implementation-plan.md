...
> **给执行者：** 使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans 按任务逐个实现。步骤使用 `- [ ]` 复选框语法追踪。

**目标：** 实现 UC-02 LoginUseCase 全栈链路：domain → outbound → usecase → infrastructure → webapi，使用真实 JWT + BCrypt + 内存存储。

**架构：** Clean Architecture 分层，遵循项目的 Presenter 模式。LoginUseCase 是公开用例（无需认证），接收用户名密码，通过 UserRepository + PasswordEncoder 验证，通过 TokenService 生成 JWT + Presenter 返回结果。
...
**产出：**
- `UserRepository`：`findByUsername(String)` → `Optional<User>`、`findById(String)` → `Optional<User>`、`save(User)` → `User`
